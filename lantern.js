...
//  vesak-lantern-3 FULLY REWRITTEN to match the uploaded reference image:
//  → neon green ribbed column (7 stacked segments)
//  → massive gold + pink lotus crown fan (3 petal tiers)
//  → stacked wide pink LED disc rings at base
//  → dense alternating pink/gold hanging balls (100+)
//  → green pointed spire with gold tip
// ============================================================================
...
export function createLantern(type = "vesak-lantern-1") {
  const colorSets = {
    ...
    // ── LANTERN 3: neon green body, gold crown, hot pink discs/ornaments ──
    "vesak-lantern-3": { paper: 0x00ff33, line: 0x003a0e, glow: 0x66ff99, trim: 0xffcc00, light: 0xff1090 },
  ...
  const styleSets = {
    "vesak-lantern-1": {
      name: "temple-tower",
      sideCount: 8, topShape: "hex", bulbShape: "sphere",
      lowerTopRadius: 0.32, lowerBottomRadius: 0.9,
      upperTopRadius: 0.82, upperBottomRadius: 0.32,
      pavilionTopCount: 4, pavilionBottomCount: 5,
      miniRadiusTop: 1.15, miniRadiusBottom: 1.1, scaleY: 1
    },
    "vesak-lantern-2": {
      name: "diamond-star",
      sideCount: 6, topShape: "diamond", bulbShape: "diamond",
      lowerTopRadius: 0.2, lowerBottomRadius: 1.05,
      upperTopRadius: 1.05, upperBottomRadius: 0.2,
      pavilionTopCount: 6, pavilionBottomCount: 6,
      miniRadiusTop: 1.35, miniRadiusBottom: 1.25, scaleY: 1.08
    },
    // ── LANTERN 3 style params ──
    "vesak-lantern-3": {
      name: "lotus-umbrella",
      sideCount: 16, topShape: "umbrella", bulbShape: "lotus",
      lowerTopRadius: 0.42, lowerBottomRadius: 0.46,
      upperTopRadius: 0.46, upperBottomRadius: 0.42,
      pavilionTopCount: 18, pavilionBottomCount: 20,
      miniRadiusTop: 1.9, miniRadiusBottom: 1.7, scaleY: 1.2
    }
  };
  ...
  lantern.userData = {
    ...
    topSpinSpeed:    type === "vesak-lantern-3" ? 0.10 : (0.24 + (seed % 5) / 100),
    middleSpinSpeed: type === "vesak-lantern-3" ? 0.07 : (0.2  + (seed % 4) / 100),
    bottomSpinSpeed: type === "vesak-lantern-3" ? 0.10 : (0.24 + (seed % 5) / 100),
  ...
  const trimMat = (emI = 0.5) => new THREE.MeshStandardMaterial({
    color: c.trim, emissive: c.trim, emissiveIntensity: emI,
    roughness: 0.35, metalness: 0.2
  });

  const solidGlow = (hex, emI = 0.8) => new THREE.MeshStandardMaterial({
  ...
  // =========================================================================
  //  LANTERN 3 — custom build (matches reference image exactly)
  // =========================================================================

  if (type === "vesak-lantern-3") {

    // helper: glowing ring (pink or gold)
    const glowRing = (parent, y, radius, thickness, col, emI = 0.9) => {
      const m = new THREE.Mesh(
        new THREE.TorusGeometry(radius, thickness, 12, 64),
        new THREE.MeshStandardMaterial({
          color: col, emissive: col, emissiveIntensity: emI, roughness: 0.3
        })
      );
      m.position.y = y;
      m.rotation.x = Math.PI / 2;
    ...
    // helper: ribbed green column segment
    const columnSeg = (parent, y, rTop, rBot, height) => {
      const seg = new THREE.Mesh(
        new THREE.CylinderGeometry(rTop, rBot, height, 16, 1, true),
        texMat(T.ribs, 16, 1, 1.6)
      );
      seg.position.y = y;
      parent.add(seg);
    };

    // ── PART 3 BOTTOM: stacked wide LED disc rings ────────────────────────

    // Three layered disc platforms — the horizontal halo stacks at base
    // widest at bottom, narrowing upward
    [
      { y: -3.15, rT: 1.40, rB: 1.50, h: 0.20 },
      { y: -2.88, rT: 1.18, rB: 1.28, h: 0.16 },
      ...
      // Gold inner dot ring
      glowRing(part3Bottom, d.y + d.h / 2 + 0.01, d.rT * 0.65, 0.016, c.trim, 0.85);
      // Pink edge ring
      glowRing(part3Bottom, d.y,                   d.rB + 0.01,  0.018, c.light, 0.8);
    });

    // 3 ribbed green column segments at bottom
    [
      { y: -2.35, rT: 0.40, rB: 0.44 },
      { y: -1.72, rT: 0.39, rB: 0.42 },
      { y: -1.10, rT: 0.38, rB: 0.41 },
    ].forEach((s, i) => {
      columnSeg(part3Bottom, s.y, s.rT, s.rB, 0.55);
      glowRing(part3Bottom, s.y + 0.28, s.rB, 0.022, c.trim, 0.9);
    });

    // ── PART 2 MIDDLE: ribbed column + massive lotus crown fan ────────────

    // 2 more ribbed green segments in middle
    [
      { y: -0.50, rT: 0.37, rB: 0.40 },
      { y:  0.12, rT: 0.36, rB: 0.39 },
    ].forEach(s => {
      columnSeg(part2Middle, s.y, s.rT, s.rB, 0.55);
      glowRing(part2Middle, s.y + 0.28, s.rB, 0.022, c.trim, 0.9);
    });

    // ── THE LOTUS CROWN FAN ──────────────────────────────────────────────
    // Matches the image: 3-tier radiating petal fan, very wide
    // Tier order bottom→top: pink inner → gold mid → gold outer

    const crownCy = 0.62; // crown center Y

    // Crown center ring
    glowRing(part2Middle, crownCy, 0.44, 0.04, c.trim, 1.1);

    // Petal helper
    const addPetalTier = (parent, count, radius, dropAngle, length, topR, col, yOff) => {
      for (let i = 0; i < count; i++) {
        const angle = (i / count) * Math.PI * 2;
        const petal = new THREE.Mesh(
          new THREE.ConeGeometry(topR, length, 6, 1),
          new THREE.MeshStandardMaterial({
            color: col, emissive: col, emissiveIntensity: 1.05, roughness: 0.3
          })
        );
        // Position at radius, then tilt outward+downward
        const px = Math.sin(angle) * radius;
        const pz = Math.cos(angle) * radius;
        petal.position.set(px, crownCy + yOff, pz);
        // Point outward (lay flat), then rotate around its own axis to face outward
        petal.rotation.z = Math.PI / 2;
        petal.rotation.y = -angle;
        // Tilt down by dropAngle
        const axis = new THREE.Vector3(Math.cos(angle), 0, -Math.sin(angle));
        petal.rotateOnAxis(axis, -dropAngle);
        parent.add(petal);
      }
    };

    // Tier 1 — outermost large gold petals (the big fan spread)
    addPetalTier(part2Middle, 26, 1.05, 0.40, 0.90, 0.075, c.trim,  0.00);
    // Tier 2 — mid gold petals (slightly inside, higher angle)
    addPetalTier(part2Middle, 22, 0.82, 0.30, 0.75, 0.062, c.trim,  0.10);
    // Tier 3 — inner pink petals (shortest, most upright)
    addPetalTier(part2Middle, 18, 0.62, 0.20, 0.58, 0.055, c.light, 0.18);
    // Tier 4 — very inner short pink uptips
    addPetalTier(part2Middle, 14, 0.44, 0.10, 0.40, 0.045, c.light, 0.22);

    // Glow sphere at crown center
    const glowMat = new THREE.MeshBasicMaterial({
      color: c.glow, transparent: true, opacity: 0.50,
      blending: THREE.AdditiveBlending, depthWrite: false
    });
    const glow = new THREE.Mesh(new THREE.SphereGeometry(0.48, 20, 20), glowMat);
    glow.position.y = crownCy;
    part2Middle.add(glow);
    U.glowShells.push({ mesh: glow, base: 0.50 });

    // Wide ambient halo
    const haloMat = new THREE.MeshBasicMaterial({
      color: c.glow, transparent: true, opacity: 0.07,
      blending: THREE.AdditiveBlending, depthWrite: false
    });
    const halo = new THREE.Mesh(new THREE.SphereGeometry(2.9, 16, 16), haloMat);
    part2Middle.add(halo);
    U.glowShells.push({ mesh: halo, base: 0.07 });

    // ── PART 1 TOP: 2 column segments + pointed green spire + gold tip ────

    // 2 top ribbed segments (narrowing upward)
    [
      { y: 1.12, rT: 0.32, rB: 0.36 },
      { y: 1.68, rT: 0.26, rB: 0.31 },
    ].forEach(s => {
      columnSeg(part1Top, s.y, s.rT, s.rB, 0.50);
      glowRing(part1Top, s.y + 0.26, s.rB, 0.020, c.trim, 0.9);
    });

    // Pink accent ring below spire
    glowRing(part1Top, 1.96, 0.30, 0.028, c.light, 1.0);

    // Green pointed spire
    const spire = new THREE.Mesh(
      new THREE.ConeGeometry(0.24, 0.58, 12),
      new THREE.MeshStandardMaterial({
        color: c.paper, emissive: c.paper, emissiveIntensity: 1.15, roughness: 0.3
      })
    );
    ...
    const tip = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({
        color: c.trim, emissive: c.trim, emissiveIntensity: 1.3, roughness: 0.2
      })
    );
    tip.position.y = 2.57;
    part1Top.add(tip);
    ...
    // ── POINT LIGHTS ─────────────────────────────────────────────────────
    [
      { y:  0.6,  col: 0x66ff88, int: 2.8, dist: 7 },  // green body light
      { y: -1.2,  col: 0xff1090, int: 2.5, dist: 6 },  // pink lower
      { y:  crownCy, col: 0xffcc00, int: 2.2, dist: 5 }, // gold crown
    ].forEach(def => {
      const L = new THREE.PointLight(def.col, def.int, def.dist, 1.6);
      L.position.y = def.y;
      core.add(L);
      U.lights.push({ light: L, base: def.int });
    });

    // ── DENSE HANGING BALLS (alternating pink/gold, matches image) ────────
    // 4 rings of balls: 2 from crown edge, 2 from disc edges
    const ballRings = [
      // from the wide crown fan edge
      { parent: part2Middle, radius: 1.40, count: 30, y:  0.15, startPink: true  },
      { parent: part2Middle, radius: 1.18, count: 26, y: -0.05, startPink: false },
      // from the top disc edge
      { parent: part3Bottom, radius: 1.42, count: 28, y: -2.92, startPink: true  },
      { parent: part3Bottom, radius: 1.20, count: 24, y: -2.72, startPink: false },
    ];

    ballRings.forEach(ring => {
      for (let i = 0; i < ring.count; i++) {
        const angle   = (i / ring.count) * Math.PI * 2;
        const isPink  = ring.startPink ? (i % 2 === 0) : (i % 2 !== 0);
        const ballCol = isPink ? c.light : c.trim;
        const beadCol = isPink ? c.trim  : c.light;
        ...
        ring.parent.add(g);
        U.balls.push({ group: g, phase: g.userData.phase });
      }
    });

    // ── SHORT RIBBONS from crown ──────────────────────────────────────────
    for (let i = 0; i < 6; i++) {
      ...
      const pts   = [
        ...
        new THREE.Vector3(0.5 * s, 0.12, 0.04),
        new THREE.Vector3(0.9 * s, 0.00, 0.0),
        new THREE.Vector3(1.1 * s, -0.22, -0.04),
    ...
    // done — skip generic build below
  }

  // =========================================================================
  //  LANTERNS 1, 2, 4, 5 — original generic build (unchanged)
  // =========================================================================
    ...
    if (style.bulbShape === "diamond") {
      bulb = new THREE.Mesh(new THREE.OctahedronGeometry(0.58, 0), texMat(T.ribs, 8, 1, 1.3));
      bulb.scale.set(1, 1.1, 1);
    } else if (style.bulbShape === "lotus") {
      bulb = new THREE.Mesh(new THREE.SphereGeometry(0.5, 28, 18), texMat(T.ribs, 14, 1, 1.28));
      bulb.scale.set(1.25, 0.75, 1.25);
    } else {
      bulb = new THREE.Mesh(new THREE.SphereGeometry(0.46, 24, 18), texMat(T.ribs, 16, 1, 1.25));
    ...
    const glowMat = new THREE.MeshBasicMaterial({
      color: c.glow, transparent: true, opacity: 0.6,
      blending: THREE.AdditiveBlending, depthWrite: false
    });
    ...
    const haloMat = new THREE.MeshBasicMaterial({
      color: c.glow, transparent: true, opacity: 0.1,
      blending: THREE.AdditiveBlending, depthWrite: false
    });
    ...
    if (style.topShape === "diamond") {
      const diamond = new THREE.Mesh(
        new THREE.OctahedronGeometry(0.55, 0), texMat(T.wheel, 6, 1, 1.15)
      );
    ...
    const tipBall = new THREE.Mesh(
      new THREE.SphereGeometry(0.07, 16, 16), solidGlow(c.glow, 0.9)
    );
    ...
    const arm = (parent, angle, R, y) => {
      const a = new THREE.Mesh(
        new THREE.CylinderGeometry(0.015, 0.015, R - 0.45, 6), trimMat(0.35)
      );
    ...
    const makeRibbon = (angle, y, flip) => {
      ...
      const m = new THREE.Mesh(
        new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts), 48, 0.05, 6, false),
    ...
    const buildBall = () => {
      ...
      for (let j = 0; j < 2; j++) {
        const bead = new THREE.Mesh(
          new THREE.SphereGeometry(0.022, 8, 8), solidGlow(c.trim, 0.6)
        );
...
export function updateLantern(lantern, delta = 0.016) {
  ...
  const flicker =
    0.84 + Math.sin(t * 8) * 0.09 + Math.sin(t * 21) * 0.05 + Math.random() * 0.05;
...
// ============================================================================
//  Procedural canvas pattern textures — unchanged
// ============================================================================
...
function patternTexture(kind, c, size) {
  ...
  const paper = css(c.paper), line = css(c.line),
        glow  = css(c.glow),  trim = css(c.trim);
