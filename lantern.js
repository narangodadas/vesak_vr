...
export function createLantern(type = "vesak-lantern-1") {
  ...
  const makePaper = (color, emissiveIntensity = 0.9, opacity = 0.76) => {
    ...
    U.patternMats.push({
      ...
      phase: U.patternMats.length * 0.6
    });
  ...
  const top = new THREE.Mesh(
    new THREE.ConeGeometry(0.88, 0.48, 8),
    trimMat
  );
  top.position.y = 0.86;
  core.add(top);
  ...
  for (const y of [0.64, -0.64, 0.02]) {
    const ring = new THREE.Mesh(
      new THREE.TorusGeometry(0.84, 0.025, 16, 100),
      trimMat
    );
    ring.position.y = y;
  ...
  lantern.position.set(0, 0.2, -3);
  lantern.scale.setScalar(0.48);
...
export function updateLantern(lantern, delta = 0.016) {
  ...
  // Rotate only the inner core. Outer world-anchor stays stable.
  if (U.core) {
    U.core.rotation.y += delta * U.spinSpeed;
    U.core.position.y = Math.sin(t * 1.1) * 0.045;
  }

  // Tiny natural sway, but not screen-following movement
  lantern.rotation.z = Math.sin(t * 0.72) * 0.018;
  lantern.rotation.x = Math.sin(t * 0.58 + 1.2) * 0.014;

  const flicker =
    0.88 +
    Math.sin(t * 8.0) * 0.12 +
    Math.sin(t * 18.0) * 0.05 +
    Math.random() * 0.04;
  ...
  U.patternMats.forEach((p) => {
    p.mat.emissiveIntensity =
      p.base + Math.sin(t * 2.5 + p.phase) * 0.16;
  });
  ...
  U.glowShells.forEach((s) => {
    ...
    s.mesh.material.opacity =
      s.base * (0.85 + Math.abs(Math.sin(t * 3)) * 0.45);
  });

  U.tassels.forEach((tassel) => {
    tassel.rotation.z =
      Math.sin(t * 2 + tassel.userData.phase) * 0.16;
  });
}
...
function patternTexture(paper, line, glow) {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    ctx.lineTo(
      size / 2 + Math.cos(a) * 96,
      size / 2 + Math.sin(a) * 96
    );
    ctx.stroke();
  }
  ...
  for (let i = 0; i < 24; i++) {
    ...
    ctx.beginPath();
    ctx.arc(
      size / 2 + Math.cos(a) * 78,
      size / 2 + Math.sin(a) * 78,
      5,
      0,
      Math.PI * 2
    );
